...
export const updateConversationMemberSettingsHandler = async (req: Request, res: Response) => {
  try {
    const conversationId = Array.isArray(req.params.conversationId)
      ? req.params.conversationId[0]
      : req.params.conversationId;
    ...
    const member = await updateConversationMemberSettings(conversationId, userId, {
    ...
    console.error("Update conversation member settings error:", error);
